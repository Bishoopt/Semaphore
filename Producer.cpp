...
int main(int argc, char* argv[])
{
    if(argc!=6)
    {
      std::cout<<"Producer: MEMKEY SEMKEY QUENUMBER(1,2,..) SLEEPTIME(us) RAND\n";
      return -1;
    }
    ...
    while(1)
    {
      Message m(rand()%3,rand()%3,rand()%3,0);
      semDown(EMPTY(queueNumber));
      semDown(MUTEX(queueNumber));
      semDown(CONSOLERW);
	std::cout<<"Producer "<<(char)('A'+queueNumber)<<": "<<m.toString()<<std::endl;
